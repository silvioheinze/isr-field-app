...
class MappingArea(models.Model):
    """Mapping area defined as one or more polygons on the map (GeoJSON Polygon or MultiPolygon)."""
    dataset = models.ForeignKey(DataSet, on_delete=models.CASCADE, related_name='mapping_areas')
    name = models.CharField(max_length=255)
    geometry = gis_models.MultiPolygonField(srid=4326)  # WGS84; single-part areas stored as one polygon
